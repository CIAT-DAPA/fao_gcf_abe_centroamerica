...
oDir <- "D:/cenavarro/bid_mag_pasar/climate_change/anomalies"
      ...
      for (metric in metrics){
        ...
        if (!file.exists(oPlot)) {
          ...
          if (var == "prec"){
            ...
            zvalues <- seq(-20, 20, 5) # Define limits
          ...
          # Save to file
          tiff(oPlot, width=1400, height=1200, pointsize=8, compression='lzw',res=200)
          ...
          + layer(sp.polygons(mask_adm1, lwd=0.7))
  ...
  for (var in varList){
    ...
    for (metric in metrics){
      ...
      if (!file.exists(oPlot)) {
        ...
        tiff(oPlot, width=2000, height=900, compression='lzw',res=200)
