...
oDir <- "X:/1.Data/Results/climate/01_baseline/hnd/evaluation_watershed_v2"
...
mask <- as(project(vect("X:/1.Data/Results/Salidas&Analysis_SWAT/Honduras/Espacial/LineaBase/subs1.shp"), crs_ref), "Spatial")
mask1 <- as(project(vect("X:/1.Data/Process/Info_Inputs_SWAT/Honduras/Choluteca/Cuencas_Drenajes/Agua_Honduras/Subcuencas.shp"), crs_ref), "Spatial")
...
for (var in varList){
  ...
  stk_crop <- stack(paste0(bDir, "/", var, "_", 1:12, ".asc"))
  ...
  # Save to file
  tiff(paste(oDir, "/plot_monthly_clim_", var, ".tif", sep=""), width=1400, height=1600, pointsize=8, compression='lzw',res=200)
  ...
  print(levelplot(plot, at = zvalues, scales = list(draw=FALSE), layout=c(4, 3), xlab="", ylab="", par.settings = myTheme, 
        ...
        + latticeExtra::layer(sp.polygons(mask1, lwd=1))
        )
...
for (var in varList){
  ...
  stk_crop <- stack(paste0(bDir, "/", var, "_", id, ".asc"))
  # stk_crop <- resample(stk_crop, raster(mask_rs), method="bilinear")
  stk_crop <- mask(crop(stk_crop, extent(mask)), mask)
  ...
  print(levelplot(plot, at = zvalues, 
        ...
        + latticeExtra::layer(sp.polygons(mask, lwd=0.3,col="gray70")) 
        + latticeExtra::layer(sp.polygons(mask1, lwd=1.2))
        )
...
for (var in varList){
  ...
  print(levelplot(plot, at = zvalues, 
        ...
        + latticeExtra::layer(sp.polygons(mask1, lwd=0.8,col="gray70")) 
