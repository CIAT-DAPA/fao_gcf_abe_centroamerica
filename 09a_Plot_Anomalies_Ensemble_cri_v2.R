...
#sspList <- c("ssp_585")
baseDir <- "//ALLIANCEDFS.ALLIANCE.CGIAR.ORG/data_cluster17V2/Climate change scenarios/CMIP6_CENTRALAMERICA/camexca_2_5min_anom_ens"
seasonDir <- "X:/1.Data/Results/climate/02_climate_change/cri_2_5min_anom_ens"
perList <- c("2050s", "2070s") #, "2090s") #,"2030s", )
varList <- c("prec", "tmin", "tmax", "tmean") 
id <- c("Ene", "Feb", "Mar", "Abr", "May", "Jun", "Jul", "Ago", "Sep", "Oct", "Nov", "Dic")
crs_ref <- crs(raster("X:/1.Data/Results/climate/02_climate_change/cri_2_5min_anom_ens/prec_ssp_126_season_avg.tif"))
...
mask <- as(project(vect("X:/1.Data/Process/Info_Inputs_SWAT/Costa_Rica/Tempisque/Cambio_Climatico/subs1.shp"), crs_ref), "Spatial")
mask_adm1 <- as(project(vect("X:/1.Data/Process/Info_Inputs_SWAT/Costa_Rica/Tempisque/Cuencas_Drenajes/Subcuencas_Oficiales_Clip_Cuenca_SWAT-FINAL.shp"), crs_ref), "Spatial")
mask_rs <- "X:/1.Data/Results/climate/01_baseline/cri/wcl_v21_30s/prec_1.tif"
resample <- T
oDir <- "X:/1.Data/Results/climate/02_climate_change/cri_evaluations_v2"
  ...
  for (per in perList){
    ...
    for (var in varList){
      ...
      for (metric in metrics){
        ...
        stk <- crop(stk, extent(mask), snap="out")
        if(resample == T){stk <- resample(stk, raster(mask_rs), method="bilinear")}
        #stk <- mask(crop(stk, extent(mask)), mask)
        stk <- mask(crop(stk, extent(mask), snap="out"), mask)
        ...
        if (!file.exists(oPlot)) {
          ...
          + latticeExtra::layer(sp.polygons(mask, lwd=0.3,col="gray70"))
      ...
      stkStd <- crop(stkStd, extent(mask), snap="out")
      if(resample == T){stkStd <- resample(stkStd, raster(mask_rs), method="bilinear")}
      #stkStd <- mask(crop(stkStd, extent(mask)), mask)
      stkStd <- mask(crop(stkStd, extent(mask), snap="out"), mask)
...
for (ssp in sspList) {
  ...
  for (var in varList){
    ...
    for (metric in metrics){
      ...
      stk <- stack(paste0(seasonDir, "/", var, "_", ssp, "_season_", metric,".tif"))
      stk_res <- resample(stk, raster(mask_rs), method = "bilinear")
      stk_ann <- stack(paste0(seasonDir, "/", var, "_", gsub("_", "", ssp), "_ann.tif")) 
