...
#sspList <- c("ssp_585")
baseDir <- "X:/1.Data/Results/climate/02_climate_change/dom_2_5min_anom_ens"
perList <- c("2050s", "2070s") #, "2090s") #,"2030s", )
varList <- c("prec", "tmin", "tmax", "tmean") 
id <- c("Ene", "Feb", "Mar", "Abr", "May", "Jun", "Jul", "Ago", "Sep", "Oct", "Nov", "Dic")
crs_ref <- crs(raster("X:/1.Data/Results/climate/02_climate_change/dom_2_5min_anom_ens/ssp_245/2050s/prec_1_avg.tif"))
...
mask_adm1 <- as(project(vect("X:/1.Data/Results/Salidas&Analysis_SWAT/Republica_Dominicana/Espacial/LineaBase/subs1.shp"), crs_ref), "Spatial")
mask_rs <- "X:/1.Data/Results/climate/00_admin_data/dom/guayubin_mao_wgs84.tif"
resample <- T
oDir <- "X:/1.Data/Results/climate/02_climate_change/dom_evaluations_v2"
    ...
    for (var in varList){
      ...
      for (metric in metrics){
        ...
        if (!file.exists(oPlot)) {
          ...
          + latticeExtra::layer(sp.polygons(mask_adm1, lwd=0.3,col="gray70"))
  ...
  for (var in varList){
    ...
    for (metric in metrics){
      ...
      if (!file.exists(oPlot)) {
        ...
        tiff(oPlot, width=1600, height=800, compression='lzw',res=200)
