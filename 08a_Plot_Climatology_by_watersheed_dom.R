...
# Set params
bDir <- "X:/1.Data/Results/climate/01_baseline/dom/wcl_v21_2_5min"
oDir <- "X:/1.Data/Results/climate/01_baseline/dom/evaluation_watersheed_v2"
...
mask <- as(project(vect("X:/1.Data/Process/Info_Inputs_SWAT/Republica_Dominicana/Cuencas_Drenajes/microcuencas_Guayubin_Mao_proj.shp"), crs_ref), "Spatial")
mask2 <- as(project(vect("X:/1.Data/Results/Salidas&Analysis_SWAT/Republica_Dominicana/Espacial/LineaBase/subs1.shp"), crs_ref), "Spatial")
mask_rs <- "X:/1.Data/Results/climate/00_admin_data/dom/guayubin_mao_wgs84.tif"
resample <- T
...
for (var in varList){
  ...
  for (i in 1:length(seasons)){
    ...
    if (!file.exists(paste(bDir,'/', var, "_", names(seasons[i]), '.tif',sep=''))){
      ...
      writeRaster(sAvg, paste(bDir,'/', var, "_", names(seasons[i]), '.tif',sep=''), overwrite=T)
    }
  }
}
...
for (var in varList){
  ...
  print(levelplot(plot, at = zvalues, scales = list(draw=FALSE), layout=c(6, 2), xlab="", ylab="", par.settings = myTheme, 
        ...
        + latticeExtra::layer(sp.polygons(mask2, lwd=0.3,col="gray70"))
        + latticeExtra::layer(sp.polygons(mask, lwd=1))
        )
...
for (var in varList){
  ...
  print(levelplot(plot, at = zvalues, 
        ...
        + latticeExtra::layer(sp.polygons(mask2, lwd=0.3,col="gray70")) 
        + latticeExtra::layer(sp.polygons(mask, lwd=1.2))
        )
...
for (var in varList){
  ...
  if (var == "prec"){
    ...
    zvalues <- seq(0, 3000, 200)
    myTheme <- BuRdTheme()
    myTheme$regions$col <- colorRampPalette(
      c("khaki1", "skyblue1", "blue", "darkblue", "darkmagenta")
    )(length(zvalues)-1)
    ...
    myTheme <- BuRdTheme()
    myTheme$regions$col <- colorRampPalette(
      c("darkblue", "yellow", "orange", "red", "darkred")
    )(length(zvalues)-1)
    
  } else if (var == "rsds"){
    
    zvalues <- seq(120, 280, by = 10)
    myTheme <- BuRdTheme()
    myTheme <- rasterTheme(region = brewer.pal("Oranges", n = 9))
    
  } else if (var == "etp"){
    
    zvalues <- seq(100, 200, by = 10)
    myTheme <- BuRdTheme()
    myTheme <- rasterTheme(region = brewer.pal("GnBu", n = 9))
    
  } else if (var == "hurs"){
    
    zvalues <- seq(60, 100, by = 5)
    myTheme <- BuRdTheme()
    myTheme <- rasterTheme(region = brewer.pal("PuBuGn", n = 9))
  ...
  # Crear el objeto plot DESPUES de modificar stk_crop
  ...
  tiff(
    paste(oDir, "/plot_annual_clim_", var, ".tif", sep=""),
    width = 1000,
    height = 1200,
    pointsize = 8,
    compression = "lzw",
  ...
  print(
    levelplot(
      ...
      latticeExtra::layer(sp.polygons(mask2, lwd = 0.8,col="gray70")) +
      latticeExtra::layer(sp.polygons(mask, lwd = 1.5))
