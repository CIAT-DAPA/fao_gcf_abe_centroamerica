...
theEntireProcess <- function(spID, OSys, inputDir, inCurClimDir, inProjClimDir, projectionList, java) {
	...
	if (!file.exists(verFile)) {
		...
		maxentApp <- paste(inputDir, "/maxent/maxent.jar", sep="")
		...
		if (file.exists(occFile)) {
			...
			if (nOcc > 0) {
				...
				backFileSwd <- paste(backoutdir, "/", spID, "_background_swd.csv", sep="")
				...
				system(
				  paste(
				    ...
				    shQuote(backFileSwd),
				    "-o",
				    shQuote(paste0(outName, "/crossval")),
				    "-P",
				    "replicates=5",
				...
				if (procSwitch) {
					...
					out <- getMetrics(paste(outName, "/crossval", sep=""), paste(spID), 5, paste(outName, "/metrics", sep=""))
