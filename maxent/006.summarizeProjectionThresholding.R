...
# setwd("D:/_scripts/giz_huila_pigcc/maxent")
...
summarize <- function(spID, inputDir, outFolder, outName, NADir, projectionList, suffix) {
...
# 
	...
	###Listing Ascii Projections
	currMean <- raster(paste(outName, "/crossval/", paste0(toupper(substr(spID, 1, 1)), substr(spID, 2, nchar(spID))), "_", suffix, "_avg.asc", sep=""))
	currStdv <- raster(paste(outName, "/crossval/", paste0(toupper(substr(spID, 1, 1)), substr(spID, 2, nchar(spID))), "_", suffix, "_stddev.asc", sep=""))
	...
	for(prj in projectionList){
	  ...
	  futMeanPR <- resample(raster(paste0(outName, "/projections/averages/",spID, "_", suffix <- gsub("/", "_", prj),  "_EMN_PR",".asc")), 
	                        currMeanPR, method = "bilinear")
		chgRasterPR <- futMeanPR - currMeanPR
		writeRaster(chgRasterPR, paste0(outName, "/projections/changes/",spID, "_", suffix <- gsub("/", "_", prj),  "_EMN_PR",".asc"), overwrite=T)
		
		futMean <- resample(raster(paste0(outName, "/projections/averages/",spID, "_", suffix <- gsub("/", "_", prj),  "_EMN",".asc")), 
		                      currMean, method = "bilinear")
		chgRaster <- futMean - currMean
		writeRaster(chgRaster, paste0(outName, "/projections/changes/",spID, "_", suffix <- gsub("/", "_", prj),  "_EMN",".asc"), overwrite=T)
