...
# crossValDir <- "Y:/1.Data/RAW/Species/mxe_outputs/sp-ceroxylon_quindiuense/crossval"
# foldSuffix <- "ceroxylon_quindiuense"
# nFolds <- 5
# outMetDir <- "Y:/1.Data/RAW/Species/mxe_outputs/sp-ceroxylon_quindiuense/metrics"
...
getMetrics <- function(crossValDir, foldSuffix, nFolds, outMetDir) {
  ...
  for (fold in 0:nFolds) {
    ...
    trainPreds <- samPredData$Cloglog.prediction[which(samPredData$Test.or.train == "train")]
    matchTrain <- rep(1, times=length(trainPreds))
    
    testPreds <- samPredData$Cloglog.prediction[which(samPredData$Test.or.train == "test")]
    ...
    logVals <- omRatesData$Corresponding.Cloglog.value
  ...
  tpThreshAvg <- crossValMxrData$X10.percentile.training.presence.Cloglog.threshold[nFolds+1]
  tpThreshStd <- sd(crossValMxrData$X10.percentile.training.presence.Cloglog.threshold[1:nFolds])
  ...
  mspsThreshAvg <- mean(crossValMxrData$Maximum.training.sensitivity.plus.specificity.Cloglog.threshold[1:nFolds])
  mspsThreshStd <- sd(crossValMxrData$Maximum.training.sensitivity.plus.specificity.Cloglog.threshold[1:nFolds])
  
  #Equal training sensitivity and specificity
  esasThreshAvg <- mean(crossValMxrData$Equal.training.sensitivity.and.specificity.Cloglog.threshold[1:nFolds])
  esasThreshStd <- sd(crossValMxrData$Equal.training.sensitivity.and.specificity.Cloglog.threshold[1:nFolds])
  
  #Balance training omission
  btoThreshAvg <- mean(crossValMxrData$Balance.training.omission..predicted.area.and.threshold.value.Cloglog.threshold[1:nFolds])
  btoThreshStd <- sd(crossValMxrData$Balance.training.omission..predicted.area.and.threshold.value.Cloglog.threshold[1:nFolds])
