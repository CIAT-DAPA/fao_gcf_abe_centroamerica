...
#sspList <- c("ssp_585")
baseDir <- "Z:/1.Data/Results/climate/02_climate_change/gtm_2_5min_anom_ens"
    ...
    for (var in varList){
      ...
      for (metric in metrics){
        ...
        if (!file.exists(oPlot)) {
          ...
          # Save to file
          tiff(oPlot, width=2000, height=1600, pointsize=8, compression='lzw',res=200)
  ...
  for (var in varList){
    ...
    for (metric in metrics){
      ...
      if (!file.exists(oPlot)) {
        ...
        tiff(oPlot, width=2400, height=1200, compression='lzw',res=200)
