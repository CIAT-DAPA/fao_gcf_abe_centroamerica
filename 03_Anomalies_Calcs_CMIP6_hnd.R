...
## Parameters
dDir <- "Z:/1.Data/Results/climate/02_climate_change/hnd_2_5min"
...
mask <- raster("Z:/1.Data/Results/climate/00_admin_data/hnd_msk_30s.tif")
scn_list <- "ssp_585"
...
#for (scn in scn_list){
  ...
  for (gcm in gcmList){
    ...
    for (period in perList) {
      ...
      oDir <- paste0(gsub("2_5min", "30s", dDir), "_anom/", scn, "/", gcm, "/", period)
      ...
      for (var in varList){
        ...
        for (mth in 1:12){
          ...
          if (!file.exists(paste(oDir, "/", var, "_", mth,".tif", sep=""))) {
            ...
            bsl_adj <- bsl
            bsl_adj[bsl_adj < 10] <- 10   # piso mínimo de 5 mm
            ...
            if (var == "prec"){
              ...
              eps <- 0.01   # mm
              anom <- del / (bsl + eps) - 1
              anom[anom > 1] <- 0     # máximo +100%
              anom[anom < -1] <- 0   # mínimo -100%
