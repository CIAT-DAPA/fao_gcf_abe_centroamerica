...
per_list <- c("2030s", "2050s", "2070s")
var_list <- c("prec", "tmax", "tmin")

msk <- "X:/1.Data/Results/climate/00_admin_data/hnd/hnd_watersheed_msk_30s.tif"

res <- "2_5min"     # resolución de los archivos de entrada
out_res <- "30s"    # resolución de los productos de salida

reg <- "hnd"

ot_dir <- "X:/1.Data/Results/climate/02_climate_change"
...
for (scn in scn_list){
  ...
  for (per in per_list){
    ...
    gcm_list <- gcm_list[gcm_list != "awi_cm_1_1_mr"] # no se usara ya que tiene solo 1 layer 
    ...
    for (gcm in gcm_list){
      ...
      ot_dir_gcm <- paste0(
        ot_dir, "/", reg, "_", out_res, "/", scn, "/", str_replace(gcm, "-", "_"), "/", per
      )
      ...
      for (var in var_list){
        ...
        if (!file.exists(paste0(ot_dir_gcm, "/", var, "_12.tif"))) {
          ...
          # -------------------------------------------------------
          # RESAMPLE TO MASK RESOLUTION (30")
          # -------------------------------------------------------
          
          stk_30s <- projectRaster(
            stk,
            mask,
            method = "bilinear"
          )
