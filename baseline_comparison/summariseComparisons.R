...
metricsSummary <- function(bDir, dataset, variable) {
	periodList <- c("ANNUAL","JJA","DJF", "MAM", "SON")
...
boxplots.cru_wcl.ISO <- function(fDir="C:/CIAT_work/CCAFS/baselineComparison/metrics", variable="prec") {
  ...
  if (!file.exists(paste(fDir,"/figures/merged",sep=""))) {dir.create(paste(fDir,"/figures/merged",sep=""))}
	...
	for (period in c("ann","djf","jja")) {
		...
		par(las=2)
		boxplot(values$N~values$ISO, ylab="Number of observations", xlab="", pch=20, col='white')
		dev.off()
		...
		par(las=2)
		boxplot(values$R2.FORCED~values$ISO, ylab="R-square", xlab="", pch=20, col='white')
		dev.off()
		...
		par(las=2)
		boxplot(values$SLOPE.FORCED~values$ISO, ylab="Slope", xlab="", pch=20, col='white')
		dev.off()
		...
		par(las=2)
		boxplot(values$ERROR~values$ISO, ylab="RMSE", xlab="", pch=20, col='white')
		dev.off()
	}
}
...
boxplots.cru_wcl.GCM <- function(fDir="C:/CIAT_work/CCAFS/baselineComparison/metrics", variable="prec") {
  ...
  if (!file.exists(paste(fDir,"/figures/merged",sep=""))) {dir.create(paste(fDir,"/figures/merged",sep=""))}
	...
	for (period in c("ann","djf","jja")) {
		...
		boxplot(values$N~values$MODEL, ylab="Number of observations", xlab="", pch=20, col='white')
		dev.off()
		...
		par(las=2, mar=c(11, 5, 1, 1))
		boxplot(values$R2.FORCED~values$MODEL, ylab="R-square", xlab="", pch=20, col='white')
		dev.off()
		...
		par(las=2, mar=c(11, 5, 1, 1))
		boxplot(values$SLOPE.FORCED~values$MODEL, ylab="Slope", xlab="", pch=20, col='white')
		dev.off()
		...
		par(las=2, mar=c(11, 5, 1, 1))
		boxplot(values$ERROR~values$MODEL, ylab="RMSE", xlab="", pch=20, col='white')
		dev.off()
	}
}
