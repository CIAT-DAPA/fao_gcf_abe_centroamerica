...
compareAndPlot <- function(msk, gcmrs, wclrs, plotit=T, plotDir=".", plotName="dummy", verbose=F) { #Main function for the comparison
  ...
  if (verbose) cat("Function applied \n")
	...
	stk <- stack(gcmrs.mod, wclrs.m, wclrs.x, wclrs.n); names(stk) <- c("GCM", "MEAN", "MAX", "MIN")
...
compareRR <- function(gcmDir, gcm, wclDir, shpDir, outDir, vn="prec", divide=T, ext=".asc", country="COL", monthList=c(6,7,8), verbose=T) {
	...
	if (!file.exists(paste(outDir, "/", lgname, sep=""))) {
		...
		mk <- raster(paste(gcmrsDir, "/prec_1.nc", sep=""))
    ...
    if (verbose) cat("Mask done! \n")
      ...
      if (verbose) cat("Month", m, "\n")
			...
			rsb <- raster(paste(gcmrsDir, "/", vn, "_", m, ".nc", sep=""))
      ...
      if (verbose) cat("Monthly comparison done! \n")
			...
			#Writing rasters
			for (l in 1:length(names(cp$RasterLayers))) {
				ro <- writeRaster(raster(cp$RasterLayers,l), paste(outDir,"/",vn,"_",m,"-",names(cp$RasterLayers)[l],sep=""), overwrite=T)
				rm(ro)
			}
    ...
    #Write those rasterLayers
		for (l in 1:length(names(cp$RasterLayers))) {
			ro <- writeRaster(raster(cp$RasterLayers,l), paste(outDir,"/",vn,"_total-",names(cp$RasterLayers)[l],sep=""), overwrite=T)
			rm(ro)
		}
