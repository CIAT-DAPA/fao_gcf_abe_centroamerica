...
st.compareAndPlot <- function(msk, gcmrs, sel.st, mName, plotit=F, plotDir=NULL, plotName=NULL, verbose=T) {
  ...
  stk <- stack(gcmrs.mod, strs.m, strs.x, strs.n); names(stk) <- c("GCM", "MEAN", "MAX", "MIN")
...
compareWSR <- function(gcmDir=NULL, gcm=NULL, shpDir=NULL, stationDir=NULL, country=NULL, variable=NULL, divide=T, months=c(6,7,8), outDir=NULL, verbose=T) {
  ...
  if (!file.exists(paste(outDir, "/", lgname, sep=""))) {
    ...
    sh <- readShapePoly(paste(shpDir, "/", country, "_adm0.shp", sep=""))
    mk <- raster(paste(gcmrsDir, "/prec_1.nc", sep="")) #mask (dummy grid, prec_01.asc by default)
    ...
    #Loading stations
    st <- read.csv(paste(stationDir, "/", st.varName, "_1970_2000_mean.csv", sep=""))
    ...
    for (m in months) {
      ...
      gcmgrid <- raster(paste(gcmrsDir, "/", variable, "_", mth, ".nc", sep=""))
      ...
      for (l in 1:length(nlayers(cp$RasterLayers)) ) {
      	ro <- writeRaster(raster(cp$RasterLayers,l), paste(outDir,"/",variable,"_",m,"-", names(cp$RasterLayer)[[l]] ,sep=""), overwrite=T)
      	rm(ro)
      }
    ...
    #Writing rasterLayers
  	for (l in 1:length(nlayers(cp$RasterLayers))) {
  		ro <- writeRaster(raster(cp$RasterLayers,l), paste(outDir,"/",variable,"_total-",names(cp$RasterLayer)[[l]],sep=""), overwrite=T)
  		rm(ro)
  	}
