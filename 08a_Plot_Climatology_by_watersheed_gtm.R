...
# Set params
bDir <- "Z:/1.Data/Results/climate/01_baseline/gtm/wcl_v21_2_5min"
oDir <- "Z:/1.Data/Results/climate/01_baseline/gtm/evaluation_v2"
...
mask <- as(project(vect("Z:/1.Data/Results/climate/00_admin_data/gtm/cuenca_salinas_gtm_mun.shp"), crs_ref), "Spatial")
mask_adm1 <- as(project(vect("Z:/1.Data/Process/Info_Inputs_SWAT/Guatemala/Rio_Salinas/Division_Administrativa/cubulco_extendido.shp"), crs_ref), "Spatial")
...
mask_rs <- "Z:/1.Data/Results/climate/00_admin_data/gtm/gtm_msk_30s.tif"
...
for (var in varList){
  ...
  # Save to file
  tiff(paste(oDir, "/plot_monthly_clim_", var, ".tif", sep=""), width=2000, height=1600, pointsize=8, compression='lzw',res=200)
  ...
  print(levelplot(plot, at = zvalues, scales = list(draw=FALSE), layout=c(4, 3), xlab="", ylab="", par.settings = myTheme, 
        ...
        + latticeExtra::layer(sp.polygons(mask, lwd=0.5, col="gray30"))
        + latticeExtra::layer(sp.polygons(mask_adm1, lwd=1.2))
...
for (var in varList){
  ...
  if (var == "prec"){
    ...
    stk_crop[which(stk_crop[]>4500)]=4500
    
    plot <- setZ(stk_crop, id_mod)
    names(plot) <- toupper(id_mod)
    
    zvalues <- seq(0, 4500, 100) # Define limits
    myTheme <- BuRdTheme() # Define squeme of colors
  ...
  tiff(paste(oDir, "/plot_seasonal_clim_", var, "_v2.tif", sep=""), width=2400, height=700, pointsize=8, compression='lzw',res=200)
...
for (var in varList){
  ...
  print(levelplot(plot, at = zvalues, 
        ...
        + latticeExtra::layer(sp.polygons(mask, lwd=0.8))
  )
