...
oStat <- "D:/cenavarro/bid_mag_pasar/climate_change/uncertainties/climate_stats_by_season.csv"
...
mask <- "S:/admin_boundaries/shp_files/CRI_adm/CRI1.shp"
poly <- readOGR(mask) 
ctrList <- poly$NAME_1
ctrList[ctrList == "Limón"] <- "Limon"
ctrList[ctrList == "San José"] <- "San Jose"
...
for(ssp in sspLs){
  ...
  tiff(PlotP, width=800, height=1200, pointsize=8, compression='lzw',res=150)
  plot(p)
  dev.off()
